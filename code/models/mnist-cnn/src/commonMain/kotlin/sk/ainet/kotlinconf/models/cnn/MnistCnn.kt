...
/**
 * CNN — a convolutional network for MNIST, the "device-first" model.
 *
 ...
 * on-device, and `:cli:runCnn` on the JVM).
 *
 * This is a *real, trained* classifier: [loadCnnWeights] fills the layers from the
 * pretrained `mnist_cnn.gguf`, and [classifyDigit] then returns an actual prediction. The
 * layer names below (`stage1.conv1`, `stage2.conv2`, `out`) are deliberately matched to the
 * tensor names inside that GGUF file — do not rename them.
 */
