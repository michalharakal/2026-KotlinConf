package sk.ainet.kotlinconf.s3_mlp
...
/**
 * Stage 3 — first end-to-end model: an MLP that approximates `y = sin(x)` on `[0, π/2]`.
 *
 ...
 * model is deterministic and needs no training loop to run. (Training the same architecture
 * live is Stage 3's "advanced" path via `sequential(trainCtx) { ... }` + `training { ... }`.)
 */
